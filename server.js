...
connection.query('SELECT user.id, user.login, vote.idUser , COUNT(`idUser`) as groupement FROM `vote`, `user` WHERE `user`.`id` = `vote`.`idUser` GROUP BY `login` , `idUser`', (err, results) => {      
    if (err) {//si erreur
      console.error('Erreur lors de la récupération des utilisateurs :', err);
      res.status(500).json({ message: 'Erreur serveur' });
      return;//permet de pas exécuter se qui suit
